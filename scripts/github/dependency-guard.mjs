...
export function shouldAutoscrubDependencyLockfiles({
  ...
  return lockfileChanges.length > 0 && dependencyManifestChanges.length === 0;
}
...
function renderAutoscrubStatusLines(status) {
  ...
  if (status.kind === "not-attempted") {
    return [
      "",
      "Auto-scrub was not attempted because this workflow can only push deterministic cleanup commits to PR branches in the main OpenClaw repository. Please remove the lockfile changes manually.",
    ];
  }
  if (status.kind === "blocked-by-dependency-manifest-fields") {
    return [
      ...
      ...status.changes.map(
        (change) => `- ${markdownCode(change.path)} changed ${change.fields.map(markdownCode).join(", ")}.`,
      ),
      "",
      "Dependency graph changes must be reviewed by security or handled by maintainers internally. Please remove the lockfile changes manually if they were accidental.",
    ];
  }
...
async function collectDependencyManifestChanges(api, { owner, repo, pullRequest, files }) {
  return collectManifestChanges(api, {
    owner,
    repo,
    pullRequest,
    files,
    fieldChanges: dependencyFieldChanges,
  });
}

async function collectManifestChanges(api, { owner, repo, pullRequest, files, fieldChanges }) {
  ...
  for (const path of manifestPaths) {
    ...
    const fields = fieldChanges(baseManifest, headManifest);
    if (fields.length > 0) {
...
async function createAutoscrubCommit(api, { owner, repo, pullRequest, lockfileChanges }) {
  const headSha = pullRequest.head.sha;
  const headRef = pullRequest.head.ref;
  const headCommit = await api.request(`/repos/${owner}/${repo}/git/commits/${headSha}`);
  const treeEntries = [];
  for (const path of lockfileChanges) {
  ...
  const commit = await api.request(`/repos/${owner}/${repo}/git/commits`, {
    method: "POST",
    body: JSON.stringify({
      message: "Remove dependency lockfile change",
      tree: tree.sha,
      parents: [headSha],
    }),
  });
  await api.request(`/repos/${owner}/${repo}/git/refs/heads/${encodeGitRef(headRef)}`, {
    method: "PATCH",
    body: JSON.stringify({
      sha: commit.sha,
      force: false,
