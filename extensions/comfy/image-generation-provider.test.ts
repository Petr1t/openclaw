...
describe("comfy image-generation provider", () => {
  ...
  it("rejects output downloads that exceed the configured media cap", async () => {
    setComfyFetchGuardForTesting(fetchWithSsrFGuardMock);
    fetchWithSsrFGuardMock
      .mockResolvedValueOnce({
        response: new Response(JSON.stringify({ prompt_id: "local-prompt-1" }), {
      ...
      .mockResolvedValueOnce({
        response: new Response(Buffer.from("x".repeat(4096)), {
          status: 200,
          headers: { "content-type": "image/png" },
        }),
        release: vi.fn(async () => {}),
    ...
    await expect(
      provider.generateImage({
        ...
        cfg: {
          ...
          agents: { defaults: { mediaMaxMb: 0.000_001 } },
        },
      }),
    ).rejects.toThrow(/Comfy output download exceeds \d+ bytes/);
  });
