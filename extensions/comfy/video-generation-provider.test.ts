...
describe("comfy video-generation provider", () => {
  ...
  it("fails closed when the downloaded output exceeds the configured media cap", async () => {
    setComfyFetchGuardForTesting(fetchWithSsrFGuardMock);
    fetchWithSsrFGuardMock
      .mockResolvedValueOnce({
        response: new Response(JSON.stringify({ prompt_id: "local-video-1" }), {
      ...
      .mockResolvedValueOnce({
        response: new Response(Buffer.from("mp4-data-that-is-too-large"), {
          status: 200,
          headers: { "content-type": "video/mp4" },
        }),
        release: vi.fn(async () => {}),
    ...
    ).rejects.toThrow(/Comfy output download exceeds/);
  });
